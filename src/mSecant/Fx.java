...
public class Fx {
	...
	public void setC(double newA,double newB,double newFa,double newFb)
	{	
			c=newB-(newFb*((newB-newA)/(newFb-newFa)));
	}
	...
	public void mSecant(String eq)
	{
		...
		setFa(fOfX(eq,getA()));
		setFb(fOfX(eq,getB()));
		// calc C >> mid point // put it after fa ,fb cause we need these values to calc C
		setC(getA(),getB(),getFa(),getFb());
		setFc(fOfX(eq,getc()));
			//print  table header 
			System.out.printf(" %-3s%-12s%-12s%-12s%-12s%-12s%-12s%-12s%-12s%-12s\n","i","A","B","C","F(a)","F(b)","F(c)","cond1","cond2","Roots");
			...
			{				
				...
				System.out.printf(" %-3d%-12.4f%-12.4f%-12.4f%-12.4f%-12.4f%-12.4f%-12b%-12b%-12.4s\n",getN(),getA(),getB(),getc(),getFa(),getFb(),getFc(),checkCond1(getFc()),checkCond2(getnOfI()),getRoot());					// send fixed number of iteration to n to update it 
				setN(getnOfI());
